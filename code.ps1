...
$logPath = "C:\Users\xi.tian\2016\Log\usingPowershell\log\"
#export user, profile, role, and report tables from SFDC using data loader, makesure the format is UTF8
$sfdctbPath = "C:\Users\xi.tian\2016\Log\usingPowershell\SFDC\"
#outPath
$outPath = "C:\Users\xi.tian\2016\Log\usingPowershell\"
...
$user = Import-Csv -Path $sfdctbPath"user.csv" -Encoding UTF8 | Select ID,USERNAME,NAME, PROFILEID, USERROLEID 
#change column names
...
#profile
$profile = Import-Csv -Path $sfdctbPath"profile.csv" -Encoding UTF8 | Select ID,NAME
$profile = $profile | Select-Object @{Expression = {$_.ID}; label='PROFILEID_p'}, @{Expression = {$_.NAME}; label='PROFILENAME'}
#role
$role = Import-Csv -Path $sfdctbPath"role.csv" -Encoding UTF8 | Select ID,NAME
$role = $role | Select-Object @{Expression = {$_.ID}; label='ROLEID_r'}, @{Expression = {$_.NAME}; label='ROLENAME'}
#report
$report = Import-Csv -Path $sfdctbPath"report.csv" -Encoding UTF8 | Select ID,NAME, DESCRIPTION
...
$final = $total4 | Select NAME, PROFILENAME, ROLENAME, REPORTNAME, DESCRIPTION, REPORTID_r, timestamp | Sort-Object timestamp
$final | Export-Csv $outPath"report_export_list.csv" -Encoding UTF8

#Step 6 
#import existing profile/permissions
#update role
$newrole = Import-Csv -Path $sfdctbPath"0829_updatedprofile.csv" -Encoding UTF8 | Select Email,Role_new, Profile_new
$total5 = Join-Object -Left $total4 -Right $newrole -Where {$args[0].USERNAME -ceq $args[1].Email} -LeftProperties "*" -RightProperties "*" -Type AllInLeft
######
$info = $total5 | Select userId, NAME, Email, PROFILENAME, ROLENAME, Profile_new, Role_new 
$final2 = $total5 | Select userId | Group-Object userId | Sort count -descending | Select-Object @{Expression = {$_.NAME}; label='NAME_userid'}, @{Expression = {$_.count}; label='Count'}
...
$total8 = $total7 | Select NAME, PROFILENAME, ROLENAME, Profile_new, Role_new, Count
$total8 | Export-Csv $outPath"report_export_cnt_list2.csv" -Encoding UTF8
